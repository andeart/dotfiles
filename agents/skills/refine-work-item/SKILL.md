...
`tracker=` is the resolved tracker, and `config_path=` is its config file, or empty when the repo
has none. Step 3 reads that path. At `10` stdout is the candidates, bare, with no `key=value` line.
...
`config_path=` means the repo has no config, so there is nothing to read.
