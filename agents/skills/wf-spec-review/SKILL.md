...
**Whether the spec is tracked** decides whether a reviewer's revision gets committed. Run `git ls-files --error-unmatch <spec path>` from the repo root, discarding output; exit `0` sets `<SPEC_TRACKED>` to `yes`, anything else sets it to `no`. A repo that gitignores all of `docs/` answers `no`; one that tracks `docs/superpowers/specs/` answers `yes`. Both shapes exist in this family, so read the exit status rather than assuming either.
...
**Spawn a sub-agent** with the opening prompt below, verbatim. Substitute only `YourName`, the spec path, the notes path, the identifier, the focus list, and `<CHECK_STATE>`. Give it nothing else about the review - no summary of earlier reviewers, no repo orientation, no account of what has already been found. The genericity of the prompt is what makes each pass holistic. Give the Agent tool call itself a `description` of exactly `Spec review: YourName` - `wf-cycle-timings` identifies and names every reviewer by matching that literal prefix against each sub-agent's own description, and a differently phrased one makes it find none.
...
```text
I have a spec written in my worktree at <ABSOLUTE SPEC PATH>. Review this spec holistically. Write your review feedback in normal markdown format to <ABSOLUTE NOTES PATH>. That path is outside the repository, which is deliberate and fine. The spec file is the only deliverable. Every point you raise must be actionable as an edit to that spec. If a suggestion can only be carried out by writing implementation code, write it into the spec as a design note instead.
...
Report, one line per reviewer: its name, whether it wrote its notes file, whether its follow-through ran or was skipped for raising no bullet items, and whether it revised the spec. Then the check state the cycle ended on, the directory the notes were written to, and the push result.
