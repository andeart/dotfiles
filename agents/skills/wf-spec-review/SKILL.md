...
**The notes directory** is a fresh `<identifier>-review-notes` subdirectory created under the session's scratchpad, the absolute path this session was given for temporary files; with none on offer, `mktemp -d` prints a usable one to use directly. Create the subdirectory rather than handing out the scratchpad root as-is - a long-lived session's scratchpad can already hold another cycle's leftovers, a prior review's notes included, and reusing it puts every reviewer's notes in among them. Whichever it is, it has to clear two conditions before the first reviewer is spawned: absolute, and outside the `$root` Step 0 resolved. Notes written under the working tree are readable by every reviewer that follows, which is the one thing this path exists to prevent, and nothing further down re-checks it. Leave the directory where it is afterwards - an agent-run `rm` trips the deletion hook these repos carry, so expect the leftover.
