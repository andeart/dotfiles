...
- `0` - stdout is two `key=value` lines, read by name and never by position: `tracker=` is the
  resolved tracker, and `config_path=` is its config file, or empty when the repo has none. Carry
  on, and keep the path for Step 3.
...
empty `config_path=` means there is none - go to "No config at all" below.
...
Reached on an empty `config_path=`, or on exit 10 with no candidates. Either way nothing resolved
anywhere - the base clone included - so there is nothing for a new file to shadow.
