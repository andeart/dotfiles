...
Do not drop any of the three for being redundant, and do not drop the `NR &&`. Each method has exactly one probe that answers it, so dropping a probe turns that method's every wrap into a false stop. The `NR &&` is what keeps the third probe from reading its own failure as a landing: `git cherry` writes errors to stderr, so a probe that cannot resolve `origin/<DEFAULT>` produces the same empty stdout as a branch whose every commit is upstream, and `landed=` is the one output that authorises Step 4's discard. That sweep also skips merge commits, so it speaks only for the non-merge commits in `mb..<FEATURE>`. `tests/wf-wrap-landing-probe.bats` pins a row per method, plus the three that have to stay silent - a branch that never merged, one carrying a commit made after the merge, and one where the probe itself could not run.
