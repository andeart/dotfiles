...
**Spawn a sub-agent** with the opening prompt below, verbatim. Substitute only `YourName`, the worktree path, the identifier, the focus list, and the resolved default branch. Give it nothing else about the review - no summary of earlier reviewers, no repo orientation, no account of what has already been found. The genericity of the prompt is what makes each pass holistic.
...
Report, one line per reviewer: its name, whether it wrote its notes file, whether its follow-through ran or was skipped for raising nothing actionable, and whether it committed a revision. Then the check state the cycle ended on, and the push result.
