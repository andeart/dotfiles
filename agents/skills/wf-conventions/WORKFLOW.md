...
- *…you merge…*
- **`/wf-wrap`** - marks it Done, tears down the worktree if there was one, returns to the default branch, pulls, deletes the branch, watches post-merge CI. Where auto-merge is armed it waits for the merge to land rather than stopping.
...
Two things worth remembering: a dirty tree stops `/wf-ship` and `/wf-wrap` by design, and `.wf.yml` in the repo root controls the states, reviewers, test commands, and whether drafts and CI-watching are on. `CONFIG.md` beside this file documents every key.
