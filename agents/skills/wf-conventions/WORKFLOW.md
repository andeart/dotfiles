...
**`wf-cycle-timings <session-dir>`** also sits outside the sequence - run it from the shell after a `/wf-spec-review` or `/wf-impl-review` cycle to report each reviewer's read-only and follow-through time from that cycle's own sub-agent transcripts.
